...
const getProcessRegistrationEmailBody = (process, currentHost, currentUser, isLeave = false) => {
  const processId = process._id,
    ...
    processReadingMaterial = process.readingMaterial,
  ...
  .moz-text-html .mj-column-per-50 { width:50% !important; max-width: 50%; }
    ...
    <body style="word-spacing:normal;">
        ...
        <div
           style="font-family:Sarabun, Arial;font-size:16px;line-height:1;text-align:center;color:rgb(50, 50, 50);"
        >${confirmedApprovalBrief}: <b>${processTitle}</b><br /></div>
...
const getInviteToPrivateProcessEmailBody = (process, currentHost, currentUser) => {
  const processId = process._id,
    processTitle = process.title,
    processBody = process.description,
    processReadingMaterial = process.readingMaterial,
  ...
  const { dear, visitPage } = tr.general;
  const { confirmedApprovalText, confirmedApprovalTextLong } = tr.invitePrivateProcess;
...
export { getProcessRegistrationEmailBody, getInviteToPrivateProcessEmailBody };
